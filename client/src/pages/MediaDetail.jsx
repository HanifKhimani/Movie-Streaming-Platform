...
// import { addFavorite, removeFavorite, addWatchlater, removeWatchlater } from "../redux/features/userSlice";
...
//   const { users, listWatchlater } = useSelector((state) => state.user);
...
const MediaDetail = () => {
  ...
  const onRemoveWatchLater = async () => {
    ...
    const watchLaterItem = listWatchlater.find(e => e.mediaId.toString() === media.id.toString());

    const { response, err } = await watchlaterApi.remove({ watchLaterId: watchLaterItem.id });

    setOnRequest(false);

    if (err) toast.error(err.message);
    if (response) {
      dispatch(removeWatchlater(watchLaterItem));
                ...
                <Stack spacing={5}>
                  ...
                  <Stack direction="row" spacing={1}>
                    ...
                    <LoadingButton
                      ...
                      sx={{
                        width: "max-content",
                        "& .MuiButton-watchLaterIcon": { marginRight: "0" }
                      }}
                      size="large"
                      startIcon={isWatchlater ? <WatchLaterIcon /> : <WatchLaterOutlinedIcon />}
                      loadingPosition="start"
                      loading={onRequest}
                      onClick={isWatchlater ? onRemoveWatchLater : onWatchLaterClick}
                    />
                    <Button
                      variant="contained"
                      sx={{ width: "max-content" }}
